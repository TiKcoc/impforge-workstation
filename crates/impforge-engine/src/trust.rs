...
//! Three-Factor Hebbian/STDP Trust Scoring for ImpForge AI Engine
//!
...
//! - **Three-Factor**: Dopamine x Novelty x Homeostasis gates plasticity
//!
//! The key equation: Dw = eta x STDP(Dt) x M(t)
//! where M(t) = dopamine(reward) x novelty(runs) x homeostasis(avg_trust)
//!
//! References:
...
///
/// M(t) = dopamine(reward) x novelty(runs) x homeostasis(avg_trust)
///
/// This is the ImpForge Engine implementation (BSL 1.1).
...
impl HebbianTrustManager {
    ...
    ///
    /// Three-Factor STDP: Dw = A+ x exp(-Dt / tau) x speed_bonus x M(t)
    /// where M(t) = dopamine x novelty x homeostasis (third factor)
    ...
    ///
    /// Three-Factor STDP: Dw = -A- x exp(-Dt / tau) x M(t)
    ...
    ///
    /// Decay formula: score_decayed = MIN + (score - MIN) * 2^(-Dt / half_life)
